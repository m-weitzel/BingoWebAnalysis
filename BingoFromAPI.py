...
class BingoBoard:
	...
	def load_board(self, goals):
		...
		board = DataFrame(columns={'row', 'goals'})
...
def pull_seed_range(startseed, save=True):
	...
	start = time.time()
	all_seeds = pull_boards(range(startseed, startseed+100))
	end = time.time()
...
#for startseed in range(100, 200, 100):
# 	pull_seed_range(startseed)
#all_seeds = pull_boards(1)
#goalcounter = Counter(all_seeds.goals.sum())
#goal_counting_df = pd.DataFrame({'goal': [d[0] for d in goalcounter.items()], 'count': list(goalcounter.values())}).set_index('goal').sort_values('count')
#goal_counting_df.to_csv(f'/home/pi/OoT-Bingo-Analysis/goals/goalstest.csv')
